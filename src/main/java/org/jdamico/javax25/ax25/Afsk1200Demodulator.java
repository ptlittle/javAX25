...
//	private float vox_threshold = 0.1f;
	...
	private void statisticsFinalize() {
		...
		// System.out.printf("%ddB avg(f0_extremes)/avg(f1_extremes) = %.2f
		// period_rel_err = %.02f\n",
	...
	public Afsk1200Demodulator(int sample_rate, int filter_length, int emphasis, PacketHandler h) throws Exception {
		...
		log.info("sample_rate {}", this.sample_rate);
		this.samples_per_bit = (float) this.sample_rate / 1200.0f;
		System.err.printf("samples per bit = %.3f\n", this.samples_per_bit);
		// this.samples_per_bit = Afsk1200Filters.bit_periods[rate_index]; // this needs
		...
		// System.out.printf("filter lengths are %d and
		...
		// f0_cos = new float[samples_per_bit]; // f0=1200Hz so we have exactly one
		// cycle.
		// f0_sin = new float[samples_per_bit];
		// f1_cos = new float[6*samples_per_bit]; // f0=2200Hz so 11 cycles fit exactly
		// in 6 bit periods.
		// f1_sin = new float[6*samples_per_bit]; // otherwise we would have had to
		// compute sin/cos
		...
		log.info("phase_inc_symbol = {}", phase_inc_symbol) ;
...
//	private boolean vox_state = false; 
	...
	protected void addSamplesPrivate(float[] s, int n) {
		...
		while (i < n) {
			...
			// ",j,f0_cos[j],f0_sin[j],f1_cos[j_f1],f1_sin[j_f1]);
			...
			if (previous_fdiff * fdiff < 0 || previous_fdiff == 0) {
				...
				if (fdiff < 0) { // last period was high, meaning f0
					...
					// (double)samples_per_bit,bits,err);
